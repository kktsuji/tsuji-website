---
title: "Google ColabでDockerを使用する"
description:
...
- DockerはGoogle Colabで実行できない。
- docker-in-colab（udocker）を使用してColabでDockerを実行できるが、GPUにアクセスできない。

## docker-in-colab（udocker）を使用する

### インストール

[drengskapur/docker-in-colab](https://github.com/drengskapur/docker-in-colab)は[indigo-dc/udocker](https://github.com/indigo-dc/udocker)をGoogle Colabにインストールするスクリプトを提供している。
...
docker-in-colabで作成されたコンテナは、GPUセットアップオプションがあるにもかかわらず、ColabのGPUにアクセスできないようである。
...
`setup --nvidia`オプションはColabで期待どおりに動作しない。
...
これはColabの制限によるものだと思われる。インターネット上でこの問題の解決策を見つけることができなかった。

## Dockerを直接インストールする

[mwufi/install_docker_in_colab.sh](https://gist.github.com/mwufi/6718b30761cd109f9aff04c5144eb885)も、Colabに直接Dockerをインストールするスクリプトを提供している。

```python
!curl -fsSL https://gist.githubusercontent.com/mwufi/6718b30761cd109f9aff04c5144eb885/raw/install_docker_in_colab.sh | sh
```

インストールは成功したように見えるが、動作しない。
...
これらのコマンドは、Colabの制限により動作しないと思われる。
