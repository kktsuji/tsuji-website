...
This post introduces the paper "A Morphology Focused Diffusion Probabilistic Model for Synthesis of Histopathology Images" written by Moghadam _et al_[^1].
...
- Authors applied the U-net based diffusion model architecture proposed by Dhariwal _et al._[^10] to histopathology images synthesizing task.
...
- Backbone of proposed method is a neural network similar to the Unet based model improved by Dhariwal _et al._[^10]
  - This[^10] is inspired from the Unet model by Ho _et al._[^17] for diffusion model.
...
- Two networks are applied to generate synthetic histopathology images.
  ...
  - Related work’s network[^24] utilized ProGAN[^20].
    - Authors slightly modified ProGAN by referring to cGANs proposed by Miyato _et al._[^28].
