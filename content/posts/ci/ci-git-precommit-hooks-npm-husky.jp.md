...
プロジェクトをセットアップした後、変更をコミットしようとするたびに、pre-commitフックがコミットを許可する前に`npm test`を実行します。`npm test`は、pre-commitチェックとして実行したい任意のコマンドに置き換えることができます。

### npmプロジェクトを初期化し、huskyをインストール

まだプロジェクトをセットアップしていない場合は、以下のコマンドで設定できます：
...
プロジェクトに既に`package.json`とhuskyが設定されている場合は、以下を実行するだけです：
